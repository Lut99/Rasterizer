...
Shader::Shader(const Shader& other) :
    ...
    entry(other.entry)
{
    DENTER("Vulkan::Shader::Shader(copy)");
    
    // Once again we rely on reload
    DMUTE("Vulkan::Shader::reload");
    this->reload();
    DUNMUTE("Vulkan::Shader::reload");
...
void Vulkan::swap(Shader& s1, Shader& s2) {
    DENTER("Vulkan::swap(Shader)");
