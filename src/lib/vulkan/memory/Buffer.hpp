...
namespace Rasterizer::Vulkan {
    ...
    public:
        ...
        void set(const Vulkan::GPU& gpu, const Vulkan::CommandBuffer& command_buffer, const Buffer& staging_buffer, VkQueue vk_queue, void* data, uint32_t n_bytes) const;
        ...
        void get(const Vulkan::GPU& gpu, const Vulkan::CommandBuffer& command_buffer, const Buffer& staging_buffer, VkQueue vk_queue, void* data, uint32_t n_bytes) const;
        ...
        void map(const Vulkan::GPU& gpu, void** mapped_memory) const;
        ...
        void flush(const Vulkan::GPU& gpu) const;
        /* Unmaps buffer's memory. */
        void unmap(const Vulkan::GPU& gpu) const;
        ...
        inline void copyto(const Vulkan::CommandBuffer& command_buffer, VkQueue vk_queue, const Buffer& destination, bool wait_queue_idle = true) const { return copyto(destination, command_buffer, vk_queue, std::numeric_limits<VkDeviceSize>::max(), 0, wait_queue_idle); }
        /* Copies this buffer's content to another given buffer on the given memory-enabled GPU queue. The given command pool (which must be a pool for the memory-enabled queue) is used to schedule the copy. Optionally waits until the queue is idle before returning. Note that the given buffer needn't come from the same memory pool. */
        void copyto(const Buffer& destination, const Vulkan::CommandBuffer& command_buffer, VkQueue vk_queue, VkDeviceSize n_bytes, VkDeviceSize target_offset = 0, bool wait_queue_idle = true) const;
