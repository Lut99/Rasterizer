...
namespace Makma3D::Rendering {
    ...
    public:
        ...
        /* Schedules a copy to the given image on the given command buffer. Also accepts a new image layout, since we need to transition to copy destination layout anyway. */
        inline void schedule_copyto(Image* destination, VkImageLayout new_layout, const Rendering::CommandBuffer* command_buffer) const { return this->schedule_copyto(destination, new_layout, VK_WHOLE_SIZE, 0, VkOffset3D({0, 0, 0}), command_buffer); }
        ...
        void schedule_copyto(Image* destination, VkImageLayout new_layout, VkDeviceSize n_bytes, VkDeviceSize source_offset, const VkOffset3D& target_offset, const Rendering::CommandBuffer* command_buffer) const;
        ...
        /* Copies this buffer's contents to the given Image, scheduling the command on the given command buffer. Because we put the image in another layout for copying, a new layout can be specified for the image. Optionally, a queue can be given to run the commands on, which will otherwise default to the first memory queue of the internal GPU. */
        inline void copyto(Image* destination, VkImageLayout new_layout, const Rendering::CommandBuffer* command_buffer, VkQueue vk_queue = nullptr, bool wait_queue_idle = true) const { return this->copyto(destination, new_layout, VK_WHOLE_SIZE, 0, VkOffset3D({0, 0, 0}), command_buffer, vk_queue, wait_queue_idle); }
        ...
        void copyto(Image* destination, VkImageLayout new_layout, VkDeviceSize n_bytes, VkDeviceSize source_offset, const VkOffset3D& target_offset, const Rendering::CommandBuffer* command_buffer, VkQueue vk_queue = nullptr, bool wait_queue_idle = true) const;
