...
void MemoryPool::copy_buffer_to_image(const Rendering::CommandBuffer& command_buffer, BufferBlock* bbuffer, ImageBlock* bimage, VkDeviceSize buffer_offset, uint32_t buffer_pitch, VkOffset3D image_offset) {
    ...
    populate_buffer_image_copy(buffer_image_copy, buffer_offset, buffer_pitch, image_offset, bimage->vk_extent);

    // Schedule the copy
    vkCmdCopyBufferToImage(
        command_buffer.command_buffer(),
        bbuffer->vk_buffer, bimage->vk_image,
        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        1, &buffer_image_copy
    );

    // FInally, update the image layout to the new one
...
void MemoryPool::schedule_transition(Image& image, VkImageLayout new_layout, const Rendering::CommandBuffer& command_buffer) {
    ...
    // Update the image reference we were given too
...
void MemoryPool::schedule_transition(image_h image, VkImageLayout new_layout, const Rendering::CommandBuffer& command_buffer) {
    ...
    if (iter == this->vk_used_blocks.end()) {
        DLOG(fatal, "Image with handle '" + std::to_string(image) + "' does not exist.");
    }
    // Now check if image
    if ((*iter).second->type != MemoryBlockType::image) {
        DLOG(fatal, "Given handle '" + std::to_string(image) + "' does not point to an image.");
    }
    ImageBlock* bimage = (ImageBlock*) (*iter).second;

    // Prepare the image barrier
    VkImageMemoryBarrier image_barrier;
    populate_image_barrier(image_barrier, bimage->vk_image, bimage->vk_layout, new_layout);
...
void MemoryPool::schedule_copy(buffer_h buffer, image_h image, VkDeviceSize buffer_offset, uint32_t buffer_pitch, VkOffset3D image_offset, const Rendering::CommandBuffer& command_buffer) {
    ...
    // Now check if image
    if ((*iter).second->type != MemoryBlockType::buffer) {
        DLOG(fatal, "Given handle '" + std::to_string(buffer) + "' does not point to a buffer.");
    }
    BufferBlock* bbuffer = (BufferBlock*) (*iter).second;

    // Fetch the image block
    std::unordered_map<memory_h, UsedBlock*>::iterator iter = this->vk_used_blocks.find(image);
    if (iter == this->vk_used_blocks.end()) {
        DLOG(fatal, "Image with handle '" + std::to_string(buffer) + "' does not exist.");
    }
    // Now check if image
    if ((*iter).second->type != MemoryBlockType::image) {
        DLOG(fatal, "Given handle '" + std::to_string(image) + "' does not point to an image.");
    ...
    // Use the blocks to schedule the copy
...
void MemoryPool::copy(buffer_h buffer, image_h image, VkDeviceSize buffer_offset, uint32_t buffer_pitch, VkOffset3D image_offset, const Rendering::CommandBuffer& command_buffer, VkQueue vk_queue, bool wait_queue_idle) {
    ...
    this->schedule_copy(buffer, image, buffer_offset, buffer_pitch, image_offset, command_buffer);
