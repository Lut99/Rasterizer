...
namespace Makma3D::Rendering {
    ...
    private:
        ...
        void copy_buffer_to_image(const Rendering::CommandBuffer& command_buffer, BufferBlock* bbuffer, ImageBlock* bimage, VkDeviceSize buffer_offset, uint32_t buffer_pitch, VkOffset3D image_offset);
    ...
    public:
        ...
        void schedule_transition(Image& image, VkImageLayout new_layout, const Rendering::CommandBuffer& command_buffer);
        ...
        void schedule_transition(image_h image, VkImageLayout new_layout, const Rendering::CommandBuffer& command_buffer);

        /* Schedules a copy from the given Buffer (handle) to the given Image (handle) on the given command buffer. */
        inline void schedule_copy(buffer_h buffer, image_h image, const Rendering::CommandBuffer& command_buffer) { return this->schedule_copy(buffer, image, 0, 0, { 0, 0, 0 }, command_buffer); }
        /* Schedules a copy from the given Buffer (handle) to the given Image (handle) on the given command buffer. An offset can be speficied for both the buffer and the image, where the image's is in three dimensions. The buffer also has an optional pitch that determines the length of each image row. */
        void schedule_copy(buffer_h buffer, image_h image, VkDeviceSize buffer_offset, uint32_t buffer_pitch, VkOffset3D image_offset, const Rendering::CommandBuffer& command_buffer);
        /* Schedules a copy from the given Buffer to the given Image (handle) on the given command buffer. */
        inline void schedule_copy(const Buffer& buffer, image_h image, const Rendering::CommandBuffer& command_buffer) { return this->schedule_copy(buffer, image, 0, 0, { 0, 0, 0 }, command_buffer); }
        ...
        void schedule_copy(const Buffer& buffer, image_h image, VkDeviceSize buffer_offset, uint32_t buffer_pitch, VkOffset3D image_offset, const Rendering::CommandBuffer& command_buffer);
        /* Schedules a copy from the given Buffer (handle) to the given Image on the given command buffer. */
        inline void schedule_copy(buffer_h buffer, Image& image, const Rendering::CommandBuffer& command_buffer) { this->schedule_copy(buffer, image, 0, 0, { 0, 0, 0 }, command_buffer); }
        ...
        void schedule_copy(buffer_h buffer, Image& image, VkDeviceSize buffer_offset, uint32_t buffer_pitch, VkOffset3D image_offset, const Rendering::CommandBuffer& command_buffer);
        /* Schedules a copy from the given Buffer to the given Image on the given command buffer. */
        inline void schedule_copy(const Buffer& buffer, Image& image, const Rendering::CommandBuffer& command_buffer) { this->schedule_copy(buffer, image, 0, 0, { 0, 0, 0 }, command_buffer); }
        ...
        void schedule_copy(const Buffer& buffer, Image& image, VkDeviceSize buffer_offset, uint32_t buffer_pitch, VkOffset3D image_offset, const Rendering::CommandBuffer& command_buffer);

        /* Immediately copies the given Buffer (handle) to the given Image (handle) on the given command buffer. */
        inline void copy(buffer_h buffer, image_h image, const Rendering::CommandBuffer& command_buffer, VkQueue vk_queue = nullptr, bool wait_queue_idle = true) { return this->copy(buffer, image, 0, 0, { 0, 0, 0 }, command_buffer, vk_queue, wait_queue_idle); }
        ...
        void copy(buffer_h buffer, image_h image, VkDeviceSize buffer_offset, uint32_t buffer_pitch, VkOffset3D image_offset, const Rendering::CommandBuffer& command_buffer, VkQueue vk_queue = nullptr, bool wait_queue_idle = true);
        /* Immediately copies the given Buffer to the given Image (handle) on the given command buffer. */
        inline void copy(const Buffer& buffer, image_h image, const Rendering::CommandBuffer& command_buffer, VkQueue vk_queue = nullptr, bool wait_queue_idle = true) { return this->copy(buffer, image, 0, 0, { 0, 0, 0 }, command_buffer, vk_queue, wait_queue_idle); }
        /* Immediately copies the given Buffer to the given Image (handle) on the given command buffer. An offset can be speficied for both the buffer and the image, where the image's is in three dimensions. The buffer also has an optional pitch that determines the length of each image row. */
        inline void copy(const Buffer& buffer, image_h image, VkDeviceSize buffer_offset, uint32_t buffer_pitch, VkOffset3D image_offset, const Rendering::CommandBuffer& command_buffer, VkQueue vk_queue = nullptr, bool wait_queue_idle = true) { return this->copy(buffer.handle(), image, buffer_offset, buffer_pitch, image_offset, command_buffer, vk_queue, wait_queue_idle); }
        /* Immediately copies the given Buffer (handle) to the given Image on the given command buffer. */
        inline void copy(buffer_h buffer, Image& image, const Rendering::CommandBuffer& command_buffer, VkQueue vk_queue = nullptr, bool wait_queue_idle = true) { this->copy(buffer, image, 0, 0, { 0, 0, 0 }, command_buffer, vk_queue, wait_queue_idle); image.vk_layout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL; }
        /* Immediately copies the given Buffer (handle) to the given Image on the given command buffer. An offset can be speficied for both the buffer and the image, where the image's is in three dimensions. The buffer also has an optional pitch that determines the length of each image row. */
        inline void copy(buffer_h buffer, Image& image, VkDeviceSize buffer_offset, uint32_t buffer_pitch, VkOffset3D image_offset, const Rendering::CommandBuffer& command_buffer, VkQueue vk_queue = nullptr, bool wait_queue_idle = true) { this->copy(buffer, image.handle(), buffer_offset, buffer_pitch, image_offset, command_buffer, vk_queue, wait_queue_idle); image.vk_layout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL; }
        /* Immediately copies the given Buffer to the given Image on the given command buffer. */
        inline void copy(const Buffer& buffer, Image& image, const Rendering::CommandBuffer& command_buffer, VkQueue vk_queue = nullptr, bool wait_queue_idle = true) { this->copy(buffer, image, 0, 0, { 0, 0, 0 }, command_buffer, vk_queue, wait_queue_idle); image.vk_layout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL; }
        /* Immediately copies the given Buffer to the given Image on the given command buffer. An offset can be speficied for both the buffer and the image, where the image's is in three dimensions. The buffer also has an optional pitch that determines the length of each image row. */
        inline void copy(const Buffer& buffer, Image& image, VkDeviceSize buffer_offset, uint32_t buffer_pitch, VkOffset3D image_offset, const Rendering::CommandBuffer& command_buffer, VkQueue vk_queue = nullptr, bool wait_queue_idle = true) { this->copy(buffer.handle(), image.handle(), buffer_offset, buffer_pitch, image_offset, command_buffer, vk_queue, wait_queue_idle); image.vk_layout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL; }
